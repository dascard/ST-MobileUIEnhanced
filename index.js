...
function ensureSettingsScaffold() {
  ...
  if (!wrapper) {
    ...
    explanation.setAttribute(
      'data-i18n',
      'Choose which drawers appear in the top toolbar. Others will show in the secondary panel.'
    )
...
function initializeMobileQuickLayout() {
  ...
  let { layoutToggleInput, customizationSection, customizationList, customizationResetButton } =
  ...
  const sanitizeIconClass = (iconClass = '') => {
    ...
    const filtered = iconClass
      .split(/\s+/)
      .map((c) => c.trim())
      .filter((c) => c && !blacklist.has(c))
  ...
  const updateLayoutMetrics = () => {
    try {
      ...
      const measuredHeight = isEnhanced
        ? Math.round(primaryWrap?.getBoundingClientRect?.().height ?? 0)
        : 0
  ...
  const createShortcutButton = (config, options = {}) => {
    ...
    if ((button.className || '').includes('mobile-quick-button'))
      primaryButtons.set(targetId, button)
    return button
  }
  ...
  const applyLayoutModeSideEffects = () => {
    ...
    if (quickToggle) {
      if (currentLayoutMode === LAYOUT_MODES.CLASSIC) quickToggle.setAttribute('hidden', '')
      else quickToggle.removeAttribute('hidden')
    }
  ...
  const syncQuickPanelState = (isOpen) => {
    ...
    if (isOpen) quickPanel.removeAttribute('hidden')
  ...
  const renderShortcuts = () => {
    ...
    const quickPanel =
      document.getElementById('mobile-quick-panel') ||
      (() => {
    ...
    if (sendForm && quickPanel.parentElement !== sendForm) {
      ...
      if (anchor && anchor.nextSibling) sendForm.insertBefore(quickPanel, anchor.nextSibling)
      else sendForm.appendChild(quickPanel)
    }

    const primaryContainer =
      document.getElementById('mobile-quick-primary') ||
      (() => {
        ...
        if (reference) parent.insertBefore(wrap, reference)
    ...
    if (topSettingsHolder) {
      ...
      if (wrap && wrap.parentElement !== topSettingsHolder) {
        ...
        if (reference) topSettingsHolder.insertBefore(wrap, reference)
        else topSettingsHolder.appendChild(wrap)
      }
    }
    ...
    const drawers = topSettingsHolder
      ? Array.from(topSettingsHolder.querySelectorAll(':scope > .drawer'))
      : []
    const configs = drawers
      .map((drawer, index) => {
        ...
        const iconElement =
          toggleElement instanceof HTMLElement
            ? toggleElement.querySelector('.drawer-icon, i')
            : drawer.querySelector('.drawer-icon, i')
        const iconClass =
          drawer.dataset.mobileIcon ||
          (iconElement instanceof HTMLElement ? iconElement.className : '') ||
          ''
        const fallbackLabel =
          drawer.dataset.mobileLabel ||
          (toggleElement instanceof HTMLElement
    ...
    const primaryConfigs = configs
      .filter((c) => c.priority === 'primary')
      .sort((a, b) => a.order - b.order)
    const secondaryConfigs = configs
      .filter((c) => c.priority !== 'primary')
      .sort((a, b) => a.order - b.order)

    primaryConfigs.forEach((cfg) => {
      createShortcutButton(cfg, {
    ...
    modeBtn.textContent = isClassic
      ? 'Switch to enhanced mobile layout'
      : 'Switch to classic layout'
    modeBtn.setAttribute(
      'data-i18n',
      isClassic ? 'Switch to enhanced mobile layout' : 'Switch to classic layout'
    )
    ...
    if (customizationSection && customizationList) {
      if (!Array.isArray(configs) || configs.length === 0) {
        ...
        sorted.forEach((cfg) => {
          ...
          if (select.querySelector(`option[value="${effective}"]`)) select.value = effective
          else select.value = 'secondary'
          select.addEventListener('change', (ev) => {
            const target = ev.currentTarget
            if (!(target instanceof HTMLSelectElement)) return
            const val = target.value
            const defaultPriority =
              target.dataset.defaultPriority ?? cfg.defaultPriority ?? 'secondary'
  ...
  rebindSettingsControls = () => {
    ...
    if (
      layoutToggleInput instanceof HTMLInputElement &&
    ...
    const detailsSection =
      customizationSection instanceof HTMLDetailsElement ? customizationSection : null
    ...
    const resetBtn =
      customizationResetButton instanceof HTMLButtonElement ? customizationResetButton : null
  ...
  if (topSettingsHolder && 'MutationObserver' in window) {
    const observer = new MutationObserver((mutations) => {
      const shouldRebuild = mutations.some((mutation) => {
        ...
        return nodes.some(
          (node) => node instanceof HTMLElement && node.classList?.contains('drawer')
        )
      })
